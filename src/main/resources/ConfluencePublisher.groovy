...
import com.github.dmn1k.asciidoc2confluence.Config
...
class ConfluencePublisher {
    ...
    ConfluencePublisher(Config config) {
        this.config = config
    }

    private void MD5(String s) {
        MessageDigest.getInstance("MD5").digest(s.bytes).encodeHex().toString()
    }

    private void trythis (Closure action) {
        try {
            action.run()
            ...
            switch (error.response.status) {
                case '401':
                    ...
                    println "please check your confluence credentials"
    ...
    private void parseAdmonitionBlock(block, String type) {
    ...
    private void uploadAttachment(String pageId, String url, String fileName, String note) {
        ...
        def api = new RESTClient(config.api)
        ...
        if (attachment.size==1) {
            ...
            if (remoteHash!=localHash) {
                ...
                http = new HTTPBuilder(config.api + 'content/' + pageId + '/child/attachment/' + attachment.results[0].id + '/data')
                println "    updated attachment"
            }
        } else {
            http = new HTTPBuilder(config.api + 'content/' + pageId + '/child/attachment')
        }
        if (http) {
    ...
    private String realTitle(String pageTitle) {
        config.pagePrefix + " " + pageTitle + config.pageSuffix
    }

    private void rewriteMarks(body) {
    ...
    private void rewriteDescriptionLists(body) {
    ...
    private void rewriteInternalLinks(body, anchors, pageAnchors) {
    ...
    private void rewriteCodeblocks(body) {
        body.select('pre > code').each { code ->
            if (code.attr('data-lang')) {
                ...
                String lang = code.attr('data-lang')
                code.before("<ac:parameter ac:name=\"language\">${lang == 'cypher' ? 'text' : lang}</ac:parameter>")
            }
            code.parent() // pre now
    ...
    private String unescapeCDATASections(String html) {
...
//body can be of type Element or Elements
    def deferredUpload = []
    private String parseBody(body, anchors, pageAnchors) {
...
// the create-or-update functionality for confluence pages
    private String pushToConfluence(String pageTitle, pageBody, String parentId, anchors, pageAnchors) {
        def api = new RESTClient(config.api)
        def headers = [
                'Authorization': 'Basic ' + config.credentials,
        ...
        def pages
        trythis {
            def cql = "space='${config.spaceKey}' AND type=page AND title~'" + realTitle(pageTitle) + "'"
            if (parentId) {
        ...
        if (page) {
            ...
            if (parentId) {
                def foreignPages
                trythis {
                    def foreignCql = "space='${config.spaceKey}' AND type=page AND title~'" + realTitle(pageTitle) + "'"
    ...
    private void pushPages(pages, anchors, pageAnchors) {
        pages.each { page ->
            println page.title
    ...
    private promoteHeaders(tree, start, offset) {
        (start..7).each { i ->
    ...
    void publish() {
        config.input.each { input ->
            ...
            dom.select('div.sect1').each { sect1 ->
                ...
                if (config.createSubpages) {
                    println "CREATE SUBPAGES"
            ...
            if (parentId) {
                println "published to ${config.api - "rest/api/"}spaces/${config.spaceKey}/pages/${parentId}"
            } else {
                println "published to ${config.api - "rest/api/"}spaces/${config.spaceKey}"
            }
        }
    }
}